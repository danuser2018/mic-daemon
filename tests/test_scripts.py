...
def test_mic_start_and_stop_scripts_without_novactl(tmp_path):
    ...
    assert "Warning: novactl is not installed or not in PATH" in result_start.stderr
    assert Path("/tmp/voice_assistant/recording.flag").exists()
    ...
    assert "Warning: novactl is not installed or not in PATH" in result_stop.stderr
    assert not Path("/tmp/voice_assistant/recording.flag").exists()
...
def test_mic_start_and_stop_scripts_with_mock_novactl(tmp_path):
    ...
    mock_novactl.write_text(f"""#!/usr/bin/env bash
echo "$@" >> "{log_file}"
exit 0
""")
